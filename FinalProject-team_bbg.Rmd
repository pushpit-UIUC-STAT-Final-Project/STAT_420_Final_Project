...
Full Additive model (with all the numerical predictor and `status`)
...
shapiro.test(resid(full_additve_model))
calc_rmse(le_tst_data$life_expectancy,
          predict(full_additve_model, newdata = le_tst_data))
```

Removing some of the least significant predictors:

infant deat v. under_5_deaths
...
calc_rmse(le_tst_data$life_expectancy,
          predict(sig_linear_model, newdata = le_tst_data))

```

```{r fig.align="center", fig.width=10, fig.height=10}
sig_linear_model <- lm(life_expectancy ~  adult_mortality +
                         under_five_deaths + bmi * status + diphtheria + hiv_aids + gdp * status +
                         income_composition_of_resources * status  + schooling * status,
                       data = non_cat_predictor_df)
summary(sig_linear_model)
par(mfrow = c(2, 2))
plot(sig_linear_model, col="darkorange", )
